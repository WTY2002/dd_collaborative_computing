 ...
 */
BIGNUM* min_PHE(vector<BIGNUM*> datas) {
    BIGNUM* res = datas[0];

    for (int i = 1; i < datas.size(); i++) {
        if (BN_cmp(res, datas[i]) > 0) {
            res = BN_dup(datas[i]);
        }
    }
 ...
 */
BIGNUM* max_PHE(vector<BIGNUM*> datas) {
    BIGNUM* res = datas[0];
...
int deal(string algoName,string fileString,string resultFilePath) {
    if (algoName == "avg") {
        ...
        cout << "data_list size: " << data_list.size() << endl;
        ...
        vector<BIGNUM*> data_list = readBIGNUMsFromFile(fileString);
        BIGNUM* min = min_PHE(data_list);
        BIGNUM* max = max_PHE(data_list);
